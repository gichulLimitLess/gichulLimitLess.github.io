...
아니, 왜냐면.. `fetch` 라는 좋은 브라우저 내장 API가 있다는 걸 알면서도.. 귀찮은 게 컸던 것 같다. 매번 `response.json()` 붙여서 json 데이터를 매번 javascript 객체로 변환 시켜야 하고.. 에러 나도 예외로 지가 안 던져주고, 타임아웃도 없고… 그래서 그냥 axios 썼다. 진짜 말 그대로, 그냥 편하니까..
...
여기서 중요한 건 Promise가 **“즉시 실행 함수”**라는 거다. new Promise()를 만드는 순간, 내부의 콜백이 바로 실행된다. 사실 그래서 위와 같이 쓰는 것보단, 특정한 함수 안에서 return 값에다가 `new Promise()` 이런 형태로 많이 사용하곤 한다. 아래와 같은 코드처럼 말이다.
...
하나 더 짚자면, Promise의 콜백은 **“비동기적으로”** 실행된다. 즉, `resolve()`가 바로 실행된다고 해서, 그 다음 `.then()`이 즉시 동작하는 건 아니다. 자바스크립트 엔진은 Promise를 **Microtask Queue**에 넣고, 현재 실행 중인 동기 코드가 모두 끝난 뒤에 실행한다. 이에 대한 너무 자세한 설명을 하게 되면, 너무 Promise 관련 글이 길어질 것 같아서*~~(이미 충분이 길어…..)~~*, 관련된 내용에 대해 설명이 잘 되어 있는 블로그 글 하나의 링크를 아래에 걸어 놓고자 한다. 해당 내용에 대해서 궁금 하다면, 아래 링크의 글을 한 번 참고해 보자.
...
오호.. 이게 나한테는 제일 익숙해. 그리고, 솔직히 제일 편하기도 해. 비동기인데 동기처럼 읽히거든. 한줄씩 읽히는 순서가 자연스럽고(왜냐면, `await` 키워드 있는 그 line에서 `resolve` 혹은, `reject` 반환 안되면, 밑으로 안 내려가거든요), 에러 처리도 `try/catch` 로 통일 되었거든. 비로소 “사람의 사고 흐름”과 코드의 실행 흐름이 일치?하기 시작한 거다.
