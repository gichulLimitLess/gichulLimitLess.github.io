...
| 제품 선택 다양성 | **제한적(1~2개 반복)**<br>→‘모노톤 패브릭 소파’라는 답변을 반복적으로 제시했음. <br>정형화된 응답 구조에는 유리하지만, <br>사용자의 조건 해석에 유연성이 부족하다는 인상을 줄 수 있음 | **조건에 따라 달라짐**<br>→동일한 입력에도 ‘스마트 리클라이너’와 <br>‘모노톤 패브릭 소파’를 상황에 따라 번갈아 제안하며 <br>더 다양한 판단을 제공해 줬지만, <br>응답 일관성에선 희생이 따랐음 |
...
| 대체 방안 | 없음 <br>(Fine-Tuning 하려면, 이 방법을 사용하는 것 밖에..) | Fine-Tuning을 애초에 지원하지 않기 때문에, <br>프롬프트 엔지니어링으로 이를 극복해야 함 |
| 적합한 사용 예 | 일관된 톤/포맷이 필수인 도메인 (고객응대, 매뉴얼형 AI) | 빠른 피드백 반복이 필요한 MVP 개발 단계 |
...
| 상황 | 추천 모델 | 이유 |
| --- | --- | --- |
| 감성적/설명 중심 UI | GPT-3.5 Turbo | 정중한 표현, 구조화된? 설명 (다소 TMI 포함될 수 있음) |
...
특정 목적에 따라 어떤 모델이 더 유리할 지.. 매우 심층적으로 테스트 코드까지 짜보며 알아봤던, 뜻깊은 시간이었다. 앞으로도 어떤 API를 사용한다고 했을 때, 단순 응답 품질만이 아니라 비용, 확장 전략(프롬프트 엔지니어링 등)까지.. 다양한 측면을 고려하며 종합적으로 어떤 게 나은지 생각해보는 습관이 필요할 것 같다!
