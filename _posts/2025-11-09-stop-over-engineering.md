...
예를 들어보자. TanStack Query를 사용 한다면 queryKey와 일치하는 캐시 데이터가 있으면 서버에 요청을 안하고 캐시된 데이터를 사용할 수 있다. 이를 조금 더 단순히 생각해보면, “TanStack Query를 사용함으로서, 서버에 요청 안 날려도 되니, 사용자들이 불필요하게 안 기다리고 빠르게 데이터를 볼 수 있다”로 생각해볼 수 있다. 난 프론트엔드 개발자라면 항상 이렇게 ‘사용자’ 입장에서 편해지면 기술 도입을 하고, 아니라면 과감히 서비스에 도입을 반려해야 한다고 생각한다.

기술 유행이 정말 빠르게 바뀌는 지금 이 시점에서, 난 위와 같은 물음을 지속적으로 해보며 기술 도입에 신중해야 하는 것이 프론트엔드 개발자로서 정말 중요한 자세일 것이다.
