...
즉, RAG는 AI한테 **“검색기 달아주기”** 하는 거라고 생각하면 된다. 나는 이 RAG라는 기술을, 자주 업데이트 되고, 세부 내용이 변하는 프론트엔드 프레임워크 및 라이브러리들에 대한 공식 문서의 링크를 주면서, “여기꺼 내용 참고해서 코드 좀 짜봐라”, 이런 식으로 프롬프트를 바꿔서 AI의 헛소리를 많이 줄일 수 있었다.

내가 체감했던 RAG의 포인트는 이거다.

- **헛소리 확실히 줄어든다** (진짜 최신 문서 기반)
- **도메인 특화 가능하다** (회사 문서, 프로젝트 위키 링크 붙여버리면 정확도 올라가겠죠?)
- **내 스트레스 줄어든다** (일일이 구글링 할 필요 없음 / 실제로 나는 이 검색 기능이 도입된 후엔, 검색 엔진으로 종종 활용하기도 함)

RAG의 정확한 기본 개념은 아래와 같다고는 하는데, 이해하기 힘들면 그냥 “AI 머릿속에 있는 거 말고, 바깥에 있는 거 가지고 와서 생각할 수 있도록 진화시키는 기술”이라고 생각하면 편할 것 같다. 

>**RAG**는 생성형 AI 모델이 답변을 생성할 때, 내부적으로 저장된 데이터 외에 **외부 데이터베이스**나 문서에서 관련 정보를 검색(Retrieval)하고, 이를 바탕으로 답변을 생성(Generation)하는 기술이다. 이는 모델이 단순히 학습된 지식에만 의존하지 않고, 최신 정보나 특정 도메인 지식을 활용할 수 있게 해준다.
